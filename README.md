...
```sh
opm index add --bundles quay.io/opdev/grafanacloud-operator-bundle:v1.0.1 --tag quay.io/opdev/grafanacloud-operator-index:1.0.1 -c docker
```

Once the index is created, we will push the index image to any repository

```sh
docker push quay.io/opdev/grafanacloud-operator-index:1.0.1
```
...
```sh
operator-sdk run bundle quay.io/opdev/grafanacloud-operator-bundle:v1.0.1
```
